...
switch ($type) {
    case 'in':
        ...
        $__ARTICLES__->UpdateStock(
            intval($newQteStock),
            intval($newQteTotal), 
            intval($newQteGiven), 
            $target_article->getId()
        );
        $__MOUVEMENT__->New(
            'e',
            $values['order'], 
            intval($values['qty']), 
            intval($target_article->getId()), 
            Utilisateurs_OBJECT_($values['cc'], 'id')->getId(), 
            Utilisateurs_OBJECT_($values['cc'], 'id')->getCentreDeCout(), 
            $values['commentary']
        );
    ...
    case 'out':
        ...
        if(intval($newQteStock) < 0) {
            ...
            $user = $values['user'] ? Utilisateurs_OBJECT_($values['user'], 'id')->getId() : 0;
            $cc = $values['user'] ? Utilisateurs_OBJECT_($values['user'],'id')->getCentreDeCout() : explode(' ', $values['cc'])[0]; 
            $__ARTICLES__->UpdateStock(intval($newQteStock),intval($newQteTotal), intval($newQteGiven), $target_article->getId());
            $__MOUVEMENT__->New('s', '', intval($values['qty']), intval($target_article->getId()),  $user,  $cc,  $values['commentary']);
            echo json_encode([$target_article->getNom(), $target_article->getQuantityStock(), $target_article->getQuantityMin(), true]);
        }
        break;
    default:
