...
class Utilisateurs
{
    ...
    public function Update(string $etablissement, string $matricule, string $nom, string $prenom, string $centreDeCout, string $centreAffection, int $id) {
		$REQUEST = 'UPDATE `utilisateurs` SET etablissement = :etablissement, matricule = :matricule, nom = :nom,prenom = :prenom, centreDeCout = :centreDeCout, centreAffection = :centreAffection, dateModification = (SELECT NOW()) WHERE `id` = :id';
        $QUERY = connectPdo()->prepare($REQUEST);
        $QUERY->execute([
            'etablissement' => $etablissement,
			'matricule' => $matricule,
